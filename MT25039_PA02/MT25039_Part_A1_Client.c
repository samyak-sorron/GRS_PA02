...
// Thread arguments including output fields for metrics
typedef struct {
    char *server_ip;
    int port;
    int duration;
    ...
    double result_latency;    // Microseconds
} ThreadArgs;
...
void *client_thread_func(void *arg) {
    ...
    // A1 Requirement: Explicit user-space buffer
    char *serialized_buffer = malloc(args->msg_size);
    ...
    while (1) {
        ...
        // --- STEP A: User-Space Copy (The extra copy) ---
        int offset = 0;
        for (int i = 0; i < 8; i++) {
            int current_chunk = (i == 7) ? (chunk_size + remainder) : chunk_size;
        ...
        // --- STEP B: Kernel Copy (send) ---
        ssize_t sent = send(sock, serialized_buffer, args->msg_size, 0);
...
int main(int argc, char *argv[]) {
    ...
    ThreadArgs thread_args[num_threads]; // Array to avoid race conditions
